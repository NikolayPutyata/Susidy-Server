...
### Реєстрація/логін — за телефоном, опційні

Ідентифікація повністю по номеру телефону, без email. `/auth/register` приймає `name`, `phoneNumber`, `password`, необов'язково `city`. `/auth/login` — `phoneNumber` + `password`. Обидва лишаються опційними для клієнта (акаунт потрібен лише для історії замовлень і персональної знижки, а не для самого замовлення). Якщо людина спершу замовляла як гість, а потім зареєструвалась тим самим номером телефону — її "гостьовий" запис користувача доповнюється паролем, а не дублюється.
